...
### 修復前 ❌
```
Hitstun ─── fixed-frame ──→ hitstun_frames
                                    ↓
                            遞減: hitstun_frames--
                                    ↓
                            持續時間: 精確 ✅

Knockback ─── delta-based ──→ hit_push_timer
                                    ↓
                            遞減: hit_push_timer -= delta
                                    ↓
                            持續時間: 不精確 ❌ 累積誤差
```
...
Knockback_Delay ────────→ knockback_delay_frames
                                    ↓
                            遞減: knockback_delay_frames--
                                    ↓
                            延遲結束 → knockback_frames = hitstun_frames

Knockback ──────────────→ knockback_frames
                                    ↓
                            遞減: knockback_frames--
                                    ↓
                            持續時間: 66 幀 (0.55秒，完全同步！)
```
