	...
	if (input_data.st_mp_pressed or input_data.st_mk_pressed) and is_valid_ground_state:
		...
		if is_crouching:
			...
			if input_data.st_mp_pressed:
				...
				is_attacking = true
				attack_type = "st_mk"
	#if not $PhysicsHandler.is_push_back:
...
func _physics_process_jump(_delta: float) -> void:
	...
	if input_data.jump_pressed and is_on_floor() and not is_dashing and not is_backdashing and not is_attacking and not is_hit and not is_knockfly and not is_blocking and not is_layground:
		...
		if world:
			$PhysicsHandler.fixed_position.y = world.FLOOR_Y - 1
			$PhysicsHandler.fixed_velocity.y = 0
		if input_data.input_dir != 0:
			var jump_speed = $PhysicsHandler.jump_horizontal_speed if input_data.input_dir * facing_direction > 0 else $PhysicsHandler.jump_horizontal_speed * 0.75
			$PhysicsHandler.fixed_velocity.x = int(jump_speed * world.SIMULATION_SCALE * input_data.input_dir)
		else:
			$PhysicsHandler.fixed_velocity.x = 0
func _compute_target_state(dir_x: float, crouch_input: bool, on_floor: bool, anim_jump_dir: float) -> String:
	...
	if is_hit:
		if not on_floor and $PhysicsHandler.is_air_hit_backjump:
			return "Jump_B"
...
func _on_hitbox_area_entered(area: Area2D) -> void:
	...
	if push_manager and push_manager.is_at_corner(target):
		...
		$PhysicsHandler.is_push_back = true
		$PhysicsHandler.push_back_timer = push_duration
		$PhysicsHandler.initial_push_back = push_duration
		$PhysicsHandler.push_back_velocity = 2.0 * corner_push_distance * world.SIMULATION_SCALE / push_duration
		$PhysicsHandler.fixed_velocity.x = int(-$PhysicsHandler.push_back_velocity * get_facing_multiplier())
...
func force_update_facing_direction() -> void:
	...
	if other:
		...
		var other_left = other.global_position.x - other.get_node("PhysicsHandler").colbox_half_width
		var other_right = other.global_position.x + other.get_node("PhysicsHandler").colbox_half_width
