...
func _ready() -> void:
	...
	Debug.log("[AI PROFILER] ✓ 性能監視器已啟用，間隔: %.1f秒" % log_interval)
...
func _print_stats() -> void:
	...
	Debug.log("\n" + "=".repeat(60))
	Debug.log("█ AI PERFORMANCE REPORT - %.1f秒" % log_interval)
	Debug.log("=".repeat(60))
	
	# FPS 和幀時間
	Debug.log("\n📊 幀速率:")
	Debug.log("  平均 FPS: %.1f" % fps)
	Debug.log("  平均幀時間: %.2f ms" % avg_frame_time)
	Debug.log("  最小幀時間: %.2f ms" % min_frame_time)
	Debug.log("  最大幀時間: %.2f ms" % max_frame_time)
	Debug.log("  樣本數: %d" % frame_times.size())
	
	# 決策計算開銷
	if not decision_times.is_empty():
		Debug.log("\n⚙️ 決策系統:")
		Debug.log("  平均決策時間: %.2f ms" % avg_decision_time)
		Debug.log("  決策開銷: %.1f%%" % decision_overhead)
		Debug.log("  決策調用數: %d" % decision_times.size())
	
	# 威脅評估開銷
	if not threat_eval_times.is_empty():
		Debug.log("\n⚔️ 威脅評估:")
		Debug.log("  平均評估時間: %.2f ms" % avg_threat_time)
		Debug.log("  威脅評估開銷: %.1f%%" % threat_overhead)
		Debug.log("  評估調用數: %d" % threat_eval_times.size())
	
	# 性能等級
	Debug.log("\n⭐ 性能等級:")
	if fps >= 58:
		Debug.log("  A+ (卓越 - 60 FPS穩定)")
	elif fps >= 55:
		Debug.log("  A (優秀 - 55+ FPS)")
	elif fps >= 50:
		Debug.log("  B (良好 - 50+ FPS)")
	elif fps >= 45:
		Debug.log("  C (尚可 - 45+ FPS)")
	else:
		Debug.log("  D (需改進 - FPS < 45)")
		if decision_overhead > 20:
			Debug.log("  💡 建議: 考慮增加決策間隔或啟用自適應間隔")
		if threat_overhead > 15:
			Debug.log("  💡 建議: 考慮優化威脅評估邏輯")
...
func _print_realtime_stats() -> void:
	...
	Debug.log("🔴 [FPS] %.1f (幀時: %.2f ms)" % [fps, avg_frame])
