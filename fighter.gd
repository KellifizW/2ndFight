...
func _ready() -> void:
	super._ready()
	# 從 PhysicsHandler 取得碰撞盒尺寸
	if collision_shape and collision_shape.shape is RectangleShape2D:
		var collision_scale = collision_shape.scale
		$PhysicsHandler.colbox_half_width = collision_shape.shape.size.x * collision_scale.x / 2.0
		$PhysicsHandler.colbox_half_height = collision_shape.shape.size.y * collision_scale.y / 2.0
	else:
...
func _physics_process(delta: float) -> void:
	...
	if (is_knockfly or is_hit) and not is_on_floor():
		var friction_amount = int($PhysicsHandler.default_air_friction * world.SIMULATION_SCALE * delta)
		if $PhysicsHandler.fixed_velocity.x > 0:
			$PhysicsHandler.fixed_velocity.x = max(0, $PhysicsHandler.fixed_velocity.x - friction_amount)
		elif $PhysicsHandler.fixed_velocity.x < 0:
			$PhysicsHandler.fixed_velocity.x = min(0, $PhysicsHandler.fixed_velocity.x + friction_amount)
		print("[DEBUG AIR FRICTION] After friction - velocity.x = ", $PhysicsHandler.fixed_velocity.x)
	# ── 【攻擊輸入檢查】保持舊版──
	...
	if is_hit or is_knockfly or is_blocking:
		...
		_update_animation_state(input_data.input_dir, input_data.crouch_pressed)
	# ── 檢查 super 後 velocity 是否被改 ──
	print("[DEBUG POST SUPER] velocity.x = ", $PhysicsHandler.fixed_velocity.x)
...
func take_hit(
	...
	if (is_holding_back or is_crouch_blocking) and is_on_floor() and not is_spmove:
		...
		$PhysicsHandler.fixed_velocity.y = 0
		if not skip_push:
			$PhysicsHandler.block_push_timer = blockstun_duration
			$PhysicsHandler.block_push_velocity = 3.0 * $PhysicsHandler.block_push_distance * world.SIMULATION_SCALE / blockstun_duration
			print("【BLOCK PUSH 設定】", name, 
				  " | timer=", $PhysicsHandler.block_push_timer, 
				  " | velocity=", $PhysicsHandler.block_push_velocity, 
				  " | duration=", blockstun_duration)
			print("[DEBUG] Block push set for ", name, " | velocity = ", $PhysicsHandler.block_push_velocity, " | timer = ", $PhysicsHandler.block_push_timer)
	...
	if not is_on_floor():
		$PhysicsHandler.jump_delay_timer = 0.0
		$PhysicsHandler.just_jumped = false
		is_jumping = false
		$PhysicsHandler.is_air_hit_backjump = false
		$PhysicsHandler.air_hit_backjump_timer = 0.0
	if should_knockfly:
		# ── Knockfly 專用處理 ──
		var params = {
			"gravity": $PhysicsHandler.default_knockfly_gravity,
			"vertical_speed": $PhysicsHandler.default_knockfly_vertical_speed,
			"horizontal_speed": $PhysicsHandler.default_knockfly_horizontal_speed,
			"duration": $PhysicsHandler.default_knockfly_duration
		}
		params.merge(knockfly_params, true)
		is_knockfly = true
		knockfly_timer = max(params.duration, min_hitstun_duration)
		$PhysicsHandler.is_immune_to_floor_snap = true
		$PhysicsHandler.floor_snap_immunity_timer = $PhysicsHandler.floor_snap_immunity_duration
		$PhysicsHandler.knockfly_gravity = params.gravity
		$PhysicsHandler.knockfly_vertical_speed = params.vertical_speed
		$PhysicsHandler.knockfly_horizontal_speed = params.horizontal_speed
		# 強制設定垂直速度（避免任何殘留跳躍速度）
		$PhysicsHandler.fixed_velocity.y = int(params.vertical_speed * world.SIMULATION_SCALE)
		$PhysicsHandler.fixed_position.y -= 1
		...
		is_jumping = true
		if not skip_push:
			$PhysicsHandler.knockfly_velocity_x = -params.horizontal_speed * world.SIMULATION_SCALE * facing_mult
		_update_animation_state(0, input_data.crouch_pressed)
	else:
		...
		hit_timer = hitstun_duration
		print("[FIXED-FRAME HITSTUN START] %s 進入 hit，%d 幀 (%.3f秒)" % [name, hit_frames, hitstun_duration])
		if not skip_push:
			$PhysicsHandler.hit_push_timer = hitstun_duration
			$PhysicsHandler.hit_push_velocity = 3.0 * $PhysicsHandler.hit_push_distance * world.SIMULATION_SCALE / hitstun_duration
			print("[DEBUG] Hit push set for ", name, " | velocity = ", $PhysicsHandler.hit_push_velocity, " | timer = ", $PhysicsHandler.hit_push_timer)
		# 空中普通攻擊：強制使用後跳邏輯
		$PhysicsHandler.is_air_hit_backjump = true
		$PhysicsHandler.air_hit_backjump_timer = $PhysicsHandler.air_hit_backjump_duration
		$PhysicsHandler.fixed_velocity.x = int(-$PhysicsHandler.air_hit_backjump_speed * world.SIMULATION_SCALE * facing_mult)
		$PhysicsHandler.fixed_velocity.y = int($PhysicsHandler.air_hit_backjump_up_speed * world.SIMULATION_SCALE)
		$PhysicsHandler.is_immune_to_floor_snap = true
		$PhysicsHandler.floor_snap_immunity_timer = $PhysicsHandler.floor_snap_immunity_duration
		$PhysicsHandler.fixed_position.y -= 2
		_update_animation_state(0, input_data.crouch_pressed)
func take_knockfly() -> void:
	...
	if not is_hit and not is_knockfly and is_on_floor():
		if is_spmove:
			move_set.stop_special_move()
		is_knockfly = true
		knockfly_timer = max($PhysicsHandler.default_knockfly_duration, min_hitstun_duration)
