...
func get_input_data() -> Dictionary:
	...
	var move_up = Input.is_action_pressed("jump" + suffix)  # Allow continuous jump input
	...
	# Check for fireball, powerkk, spnk, and dp sequences
	...
	if input_manager:
		...
		if player_id == "p2" and input_manager.check_spnk_input():
			spm1_pressed = true
			st_mk_pressed = false  # Suppress normal st_mk
