...
static func play_hit_sound(owner_node: Node, attack_type: String, damage: float) -> bool:
	"""
	播放「擊中」音效：優先使用攻擊類型專屬節點（LP/MP/HP/LK/MK/HK SoundPlayer），
	找不到時回退到舊有的 HitSoundPlayer / HeavyHitSoundPlayer。
	"""
