...
# === Generic special move starter ===
func _start_special(
	move_name: String,
	player_id_req: String,
	damage: float,
	...
	# 開始特殊招式時忽略推擠（關鍵修復）
	if "skip_pushbox" in parent:
		parent.skip_pushbox = true
...
func stop_special_move() -> void:
	if not (is_powerkk or is_spnk or is_fireball or is_super or is_dp or is_hdk): return
	
	# 特殊招式結束時恢復推擠
	if "skip_pushbox" in parent:
		parent.skip_pushbox = false
