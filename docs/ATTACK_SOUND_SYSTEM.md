...
> 改動日期：2026-08-27
> 相關檔案：`scripts/combat/AttackSoundResolver.gd`、`scripts/combat/handlers/HitResponseHandler.gd`、`scripts/core/player.gd`
...
| 角色場景未加設新節點（例如 `WOO.tscn`） | 擊中音效回退到 `HitSoundPlayer` / `HeavyHitSoundPlayer`；揮空音效靜默略過 |
...
1. 在角色 `.tscn` 的根節點下加入 9 個 `AudioStreamPlayer`：
