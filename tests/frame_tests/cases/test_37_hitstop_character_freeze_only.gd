...
func run() -> bool:
	...
	check(sprite != null, "P2 應有 AnimatedSprite2D 視覺節點")
	if sprite == null:
		return false
	var sprite_base: Vector2 = sprite.position

	var slowmo = p2.slow_mo_controller
	check(slowmo != null, "P2 應有 slow_mo_controller")
	if slowmo == null:
		return false
	...
	check(not movement_violated,
		"Both players' fixed_position must be completely frozen during hitstop")
	...
	check(Engine.time_scale == 1.0, "Engine.time_scale should be 1.0 after hitstop")
	check(p1.animation_player and p1.animation_player.speed_scale == 1.0,
		"P1 AnimationPlayer speed_scale should be restored to 1.0")
	check(p2.animation_player and p2.animation_player.speed_scale == 1.0,
		"P2 AnimationPlayer speed_scale should be restored to 1.0")
	...
	check(recovered, "P2 should recover from hitstun normally after hitstop")
