...
func get_ai_input() -> Dictionary:
	...
	if current_state == "defend" or (in_danger and opponent_attacking):
		...
		if can_attack:  # 穩定攻擊輸出
			attack_pressed = true
			damage = 10.0
			attack_type = "attack"
			state_timer = 0.4
			if opponent_blocking and randf() > 0.5:  # 50% 機率連招用特殊招
				spm1_pressed = true
				damage = 20.0
				print("Debug: Combo special attack triggered for AI")
		else:
